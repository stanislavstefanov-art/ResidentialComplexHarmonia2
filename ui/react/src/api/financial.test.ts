...
test('getPeriodSummary fetches /financial-summary?period=', async () => {
  ...
  expect(fetch).toHaveBeenCalledWith(`${BASE}/financial-summary?period=2026-07`, expect.any(Object));
});

test('getMyCharges fetches /maintenance-fees/charges', async () => {
  mockFetch([]);
  await getMyCharges();
  expect(fetch).toHaveBeenCalledWith(`${BASE}/maintenance-fees/charges`, expect.any(Object));
});

test('getMyPayments fetches /payments', async () => {
  mockFetch([]);
  await getMyPayments();
  expect(fetch).toHaveBeenCalledWith(`${BASE}/payments`, expect.any(Object));
});
